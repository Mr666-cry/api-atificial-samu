...
class MimoAI {
  ...
  async sendMessage(params = {}) {
    const { prompt, messages = [], model = this.defaultModel, onStream = null } = params;
    ...
    return new Promise((resolve, reject) => {
          ...
          if (cleanLine.startsWith('data: ')) {
            ...
            try {
              ...
              if (deltaContent) {
                ...
                if (onStream) onStream(deltaContent);
              }
            } catch {}
          }
        }
      ...
      apiResponse.data.on('error', (err) => reject(err));
    });
  }
}
...
module.exports = async (req, res) => {
  ...
  // Endpoint GET untuk mengambil daftar model
  if (req.method === 'GET') {
    const models = await client.fetchModels();
    return res.status(200).json({ status: true, models });
  }

  // Endpoint POST untuk eksekusi chat completion
  if (req.method === 'POST') {
    try {
      const { prompt, messages = [], model = 'xiaomi/mimo-v2.5-pro', stream = false } = req.body || {};

      if (!prompt && messages.length === 0) {
        return res.status(400).json({ status: false, message: 'Parameter prompt atau messages wajib diisi.' });
      }

      // Mode Streaming (Event-Stream SSE)
      if (stream) {
      ...
      // Mode Normal (JSON Output)
      const result = await client.sendMessage({ prompt, messages, model });
